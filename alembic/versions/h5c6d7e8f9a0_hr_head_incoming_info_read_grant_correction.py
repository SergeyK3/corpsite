...
_ROLE_CODE = "HR_HEAD"
_GRANT_REASON = "h5c6d7e8f9a0: HR_HEAD Incoming Information read grant correction"
    ...
    op.execute(
        ...
        BEGIN
            ...
            IF v_permission_count <> 1 THEN
                RAISE EXCEPTION
                    'h5c6d7e8f9a0 requires exactly one permission with code %, found %',
                    '{_PERMISSION_CODE}', v_permission_count;
            END IF;

            IF v_permission_active IS DISTINCT FROM TRUE THEN
                RAISE EXCEPTION
                    'h5c6d7e8f9a0 requires active permission %',
                    '{_PERMISSION_CODE}';
            END IF;
            ...
            IF EXISTS (
                ...
                RAISE EXCEPTION
                    'h5c6d7e8f9a0 found a flagged-active but non-effective grant for role %',
                    v_role_id;
            END IF;
