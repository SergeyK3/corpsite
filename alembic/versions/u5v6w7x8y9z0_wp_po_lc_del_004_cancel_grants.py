"""WP-PO-LC-DEL-004 — bind cancel permissions to HR_HEAD operational contour."""
...
def upgrade() -> None:
    for permission_code in _CANCEL_PERMISSIONS:
        op.execute(
            f"""
            ...
            SELECT
                ...
                'WP-PO-LC-DEL-004: HR head cancel permissions ({permission_code})'
    ...
    for permission_code in _CANCEL_PERMISSIONS:
        op.execute(
            ...
            WHERE g.access_role_id = ar.access_role_id
              ...
              AND r.code = 'HR_HEAD'
              AND g.reason = 'WP-PO-LC-DEL-004: HR head cancel permissions ({permission_code})'
            """
        )
