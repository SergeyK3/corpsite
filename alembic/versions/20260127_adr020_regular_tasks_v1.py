...
def upgrade() -> None:
    ...
    if not _col_exists("regular_tasks", "assignment_scope"):
        op.add_column("regular_tasks", sa.Column("assignment_scope", sa.Text(), nullable=False, server_default=sa.text("'functional'")))
    ...
    if not _col_exists("regular_tasks", "schedule_params"):
        op.add_column(
            "regular_tasks",
            sa.Column("schedule_params", sa.dialects.postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        )

    if not _col_exists("regular_tasks", "create_offset_days"):
        op.add_column("regular_tasks", sa.Column("create_offset_days", sa.Integer(), nullable=False, server_default=sa.text("0")))

    if not _col_exists("regular_tasks", "due_offset_days"):
        op.add_column("regular_tasks", sa.Column("due_offset_days", sa.Integer(), nullable=False, server_default=sa.text("0")))
    ...
    if not _col_exists("regular_tasks", "created_at"):
        op.add_column("regular_tasks", sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")))

    if not _col_exists("regular_tasks", "updated_at"):
        op.add_column("regular_tasks", sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")))
    ...
    # 4) regular_task_runs — создаём, если нет
    ...
    if _table_exists("tasks") and _col_exists("tasks", "regular_task_id") and _col_exists("tasks", "period_id") and _col_exists("tasks", "executor_role_id"):
