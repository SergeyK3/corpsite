...
Основной сценарий сброса забытого пароля выполняется через учётную запись, а не
через кадровую карточку:

1. Сотрудник нажимает «Забыли пароль?» на странице входа.
2. Система предлагает обратиться к системному администратору и сообщить login.
3. Администратор открывает «Кабинет системного администратора → Управление
   доступом», находит User по login, имени или роли и после подтверждения
   выдаёт временный пароль.
4. Сотрудник входит с временным паролем и обязан установить новый.

Связь с Employee для этого сброса не обязательна. `/profile` предназначен для
самостоятельной смены известного пароля. Кадровая карточка → «Доступ» остаётся
диагностическим экраном, а не основным интерфейсом сброса.
