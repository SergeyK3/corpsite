...
- заполнение уже materialized личных карточек из ранее импортированных данных контрольного списка;
...
Dry-run не делает записей и возвращает по cohort:
...
Dry-run является основанием для решения HR_HEAD о pilot/full run, но не является per-employee manual confirmation.
...
Full run обрабатывает все eligible записи только данного этапа:

- отдельный feature flag или явный enable/run control обязателен для каждого этапа;
- один Employee/Person обрабатывается в собственной транзакции;
- ошибка одного сотрудника не откатывает ранее успешные результаты других;
...
- после interruption запуск продолжает безопасно только ещё не завершённые записи.

Непустые отличающиеся canonical values никогда автоматически не перезаписываются: они остаются conflicts в отчёте.

### 5.6. Итоговый отчёт

Итог этапа содержит counts: найдено, перенесено, already existed, skipped, conflict и error. Для проблемных строк включаются только технические identifiers и classification; ФИО, полный ИИН, raw source text и иные избыточные персональные данные не дублируются в техническом отчёте.

### 5.7. Закрытие и переход
...
Цель — сформировать cohort, а не переносить section data. Проверяются active Employee, `employees.person_id`, Person status и materialization PPR. Отсутствующие или некорректные связи исправляются только через уже реализованный ADR-065 contour. Сотрудники без Person, с ambiguous identity, deleted/rebinding source или неmaterialized карточкой без разрешённого пути включаются в preflight report, но не в migration cohort.

### Этап 1. Общие сведения

Переносятся подтверждённые ФИО и отдельные фамилия/имя/отчество, ИИН, дата рождения и другие general fields, которые действительно присутствуют в control-list source. Technical basis — WP-PPR-IDENTITY-001. При выполнении established criteria lifecycle materializes PPR и переводит её в `COLLECTING`; это не заявление о полноте всей карточки.
