...
| `CANCELLED` | Run прекращён до принятия; причина и состояние уже обработанных участников остаются доступными для отчёта. |
...
### 5.6. Full run, остановка и продолжение
...
`stage_run_id`, stage, технического Employee/Person ID и версии входного snapshot.
...
либо повторяющуюся проблему программно. Продолжение использует тот же `stage_run_id`:
сначала повторно обрабатывается сотрудник на error-позиции, и лишь после его успеха
обрабатывается следующий. Ранее `COMPLETED` участники не изменяются повторно;
идемпотентность предотвращает дублирование.
