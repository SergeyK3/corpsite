...
Runs: ready/accepted replay `19`; review-blocked `20`; participant pause `21`;
acceptance pause `22`; original stale history `23`; replacement stale smoke
`32` (Stage 0 cohort `108`). Run 32 and its replay return the exact safe
`STAGE2_RESUME_STALE`, remain paused, create no PMF run, canonical education,
