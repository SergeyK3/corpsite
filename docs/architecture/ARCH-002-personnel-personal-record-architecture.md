...
-   Architectural Invariant: Personal Card is the Central HR Aggregate
-   Non-goals
...
> **Терминология:** в пользовательском контуре — **Личная карточка по учёту кадров**;
> в предметной модели — **Personnel Personal Record (PPR)** / **Личный листок по учёту кадров**
> ([ADR-054](../adr/ADR-054-personnel-personal-record-aggregate-model.md), [WP-HR-CARD-002](./WP-HR-CARD-002-unified-personnel-record-card.md)).
> Настоящий раздел фиксирует архитектурный инвариант: **один центральный агрегат о человеке** и
> **границу между сведениями о человеке и кадровыми процессами**, воздействующими на него.

### 1. Центральный агрегат HR-системы

**Личная карточка** (представление PPR) является **центральным агрегатом HR-системы** —
**System of Record о человеке**, а не одним из кадровых процессов.

- PPR — самостоятельный предметный объект кадрового контура (§«Решение», INV-2, INV-3).
- Личная карточка — primary interactive representation этого объекта (Composite View; INV-5, PPR-REP-001).
- Контур «Кадровые процессы» (`/directory/personnel/*`) — **операционный HR-contour** для процессов,
  которые **изменяют или дополняют** состояние человека, но **не являются частью** агрегата личной карточки.

### 2. Кадровые процессы вокруг карточки, не внутри неё

Все кадровые процессы **работают вокруг** личной карточки и **изменяют её состояние** (или связанные
bounded contexts), но **не входят в состав** агрегата личной карточки как его разделы или подсистемы.

| Процесс | Отношение к личной карточке |
|---------|----------------------------|
| Кадровые обращения | Операционный workflow; может ссылаться на Person/PPR, не является разделом PPR |
| Предполагаемое трудоустройство | Pre-hire intent; transitional envelope до HIRE; не раздел биографии |
| Кадровые приказы | Изменяют Employment Relationship (INV-9); не master-storage PPR |
| Адаптация | Employee onboarding workflow; не раздел личного листка |
| Импорт | Bootstrap / staging channel (INV-8); не заменяет PPR |
| MRD (Monthly Reference Data) | Сверка и controlled migration; не aggregate boundary |
| Прочие HR-процессы | Journal, enrollment, migration wizard и т.д. — adjacent contours |

**Следствие для UI:** монтаж workflow-секций на экране личной карточки допустим как **Composite View**
(принцип 8, INV-5), но **не создаёт** новый доменный объект и **не переносит** ownership процесса
внутрь PPR.

### 3. Разделы личной карточки — только сведения о человеке и его кадровом состоянии

Разделы личной карточки содержат **только сведения, описывающие человека и его кадровое состояние**.
На текущий момент (нормативный перечень UI-секций):

| Раздел | Содержание |
|--------|------------|
| Общие сведения | Персональные и базовые кадровые идентификаторы |
| Образование | Сведения об образовании |
| Обучение | Профессиональное обучение и повышение квалификации |
| Родственники | Близкие родственники |
| Воинский учёт | Сведения о воинской обязанности и учёте |
| Трудовая биография | Трудовая деятельность до и вне текущего Employment Relationship |
| Трудовая деятельность | Текущее кадровое состояние / занятость (projection Employment Relationship) |
| История изменений | Хронология событий личной карточки (PPR events) |

Секции, перечисленные в §2 (обращения, приказы, адаптация, intended employment и др.), **не входят**
в этот перечень core-разделов, даже если временно отображаются на том же экране.

### 4. Один доменный объект — разные представления по ролям

Личная карточка имеет **единый доменный объект** (PPR, `person_id` в Phase 1), но может иметь
**разные представления** в зависимости от роли пользователя (сотрудник, кадровик, руководитель и т.д.):

- различия в **видимости секций**, **режиме read/edit** и **доступных command surfaces**;
- представления **не создают** новые доменные объекты, aggregate ID или parallel stores (PPR-REP-001).

### 5. Критерий проектирования

Основной критерий при проектировании функциональности HR-контура:

| Вопрос | Принадлежность |
|--------|----------------|
| Функциональность **описывает человека** (identity, biography, credentials, family, military, event history, текущее кадровое состояние как projection) | **Личная карточка** (PPR aggregate + Composite View) |
| Функциональность **описывает процесс воздействия на человека** (заявление, приказ, onboarding checklist, import batch, migration run, MRD review) | **Кадровые процессы** (adjacent bounded context / workflow) |

При сомнении: **process ownership** определяет контур; **person-owned facts** — PPR.

**Связь с существующими инвариантами:** настоящий раздел **не заменяет** INV-1 … INV-9; уточняет
границу между **центральным агрегатом о человеке** и **операционными HR-процессами** для UI и
...
| Правила | Архитектурные принципы, Архитектурные инварианты, Architectural Invariant: Personal Card is the Central HR Aggregate |
