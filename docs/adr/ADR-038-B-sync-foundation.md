...
Статус: Draft (Stage 0 — design only)  
Дата: 2026-06-17  
...
**Scope Stage 0:** архитектурный дизайн и план. Код, UI, Conflict Engine, bidirectional sync — **не входят**.
...
```json
{
  ...
  "created_at": "2026-06-17T14:30:22+05:00",
  "source_instance_id": "vps-pilot",
  "source_organization": "City Hospital Pilot",
  "export_scope": "hr_import_overrides_v1",
  "files": [
    {"name": "metadata.json", "required": true, "record_format": "json"},
    {"name": "employees.jsonl", "required": true, "record_format": "jsonl"},
    {"name": "employee_import_profile_overrides.jsonl", "required": true, "record_format": "jsonl"},
    {"name": "department_recoding.jsonl", "required": false, "record_format": "jsonl"},
    {"name": "org_units.jsonl", "required": false, "record_format": "jsonl"}
  ],
  "record_counts": {
    "employees": 37,
    "employee_import_profile_overrides": 12
  }
}
```
...
| `max_reader_version` | да | Верхняя граница совместимости |
| `created_at` | да | ISO 8601 timestamp экспорта |
| `source_instance_id` | да | Стабильный ID экземпляра (env/config) |
| `source_organization` | да | Человекочитаемое имя организации |
| `export_scope` | да | Набор сущностей (`hr_import_overrides_v1`, …) |
| `files` | да | Манифест файлов с `required` flag |
| `record_counts` | да | Контрольные суммы количества записей |
...
| Optional file отсутствует | **OK** (если `required: false`) |
| Required file отсутствует | **Reject** |

Reader version задаётся константой в `app/services/sync/` (Phase B.1).
...
| | |
|---|---|
| **Объём** | Read-only diff: new/update/skip/conflict/orphan counts; JSON/Markdown report; no DB writes |
| **Зависимости** | B.1, B.3 logic (shared diff module) |
| **Риски** | Duplication with Phase C — keep preview minimal |
| **Done when** | `import --preview package.zip` prints actionable report; scenarios A–D classified |
...
После утверждения данного ADR:
1. Начать **B.1 Package Format** (schemas + validator).
2. Commit Phase A / A.1 (working tree должен быть clean перед merge).
3. CI gate: ADR-038 test subset green.
