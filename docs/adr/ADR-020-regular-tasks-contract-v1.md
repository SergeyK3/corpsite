...
### Метаданные происхождения задачи (2026-06-21)

При создании или дедупликации экземпляра генератор дописывает в `tasks.description` структурированный блок (не перезаписывая существующий текст):

```
---
Источник: Автоматический запуск регулярной задачи | Догоняющий запуск регулярной задачи
ID запуска: <run_id>
Дата возникновения задачи: YYYY-MM-DD
Тип запуска: автоматический | догоняющий
Период: <label>   # только для catch-up
---
```

Дубликат блока для того же `run_id` не добавляется. При дедупликации блок дописывается к существующей задаче; копия также хранится в `regular_task_run_items.meta.origin_metadata_text`.
