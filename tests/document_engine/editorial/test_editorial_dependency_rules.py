# tests/document_engine/editorial/test_editorial_dependency_rules.py
"""Dependency direction guard for UDE-010 editorial runtime."""
...
EDITORIAL_ROOT = REPO_ROOT / "app" / "document_engine" / "editorial"

FORBIDDEN_IN_EDITORIAL = (
    "app.db",
    "app.directory",
    "app.api",
...
FORBIDDEN_SUBSTRINGS_IN_EDITORIAL = (
    "personnel_orders",
    "personnel_order",
    "operational_orders",
    "operational_order",
...
def test_editorial_runtime_has_no_forbidden_dependencies() -> None:
    modules = list(_iter_modules(EDITORIAL_ROOT, "app.document_engine.editorial"))
    assert modules, "editorial package should exist"
    violations = _violations_for_modules(
        modules,
        forbidden_prefixes=FORBIDDEN_IN_EDITORIAL,
        forbidden_substrings=FORBIDDEN_SUBSTRINGS_IN_EDITORIAL,
    )
    assert violations == []


def test_editorial_runtime_uses_read_layer() -> None:
    modules = list(_iter_modules(EDITORIAL_ROOT, "app.document_engine.editorial"))
    has_read_import = False
    for module_name in modules:
        for imported in sorted(_collect_imports(module_name)):
            if imported.startswith("app.document_engine.read_"):
                has_read_import = True
                break
    assert has_read_import, "editorial runtime should consume read layer"


def test_editorial_runtime_does_not_import_personnel_services() -> None:
    modules = list(_iter_modules(EDITORIAL_ROOT, "app.document_engine.editorial"))
...
def test_personnel_orders_runtime_does_not_import_editorial() -> None:
    personnel_paths = [
    ...
    for path in personnel_paths:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if "document_engine.editorial" in alias.name:
                        violations.append(f"{path.name} imports {alias.name}")
            elif isinstance(node, ast.ImportFrom) and node.module:
                if "document_engine.editorial" in node.module:
