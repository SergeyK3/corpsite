...
from app.services.ppr_migration_status_report_service import list_universes, matrix
...
def _fixture(conn):
    ...
    assert projection.rebuild_universe(conn, universe_id=universe) == 6
...
def test_universe_visibility_scope_filtering_pagination_and_counts():
    ...
    try:
        ...
        assert set(result["items"][0]["cells"]) == {"general", "education", "training"}
        assert sum(x["count"] for x in result["counts"]) == 3
...
def test_filters_search_safe_payload_and_stable_pagination():
    ...
    try:
        ...
        assert set(filtered["items"][0]["cells"]) == {"general", "education", "training"}
...
def test_report_query_count_is_bounded_for_rows_pagination_and_aggregation():
    ...
    try:
        ...
        assert result and len(statements) == 4
    finally:
