...
from app.api.ppr_stage0_cohort_schemas import Stage0BlockerListOut, Stage0FreezeOut, Stage0FreezeRequest, Stage0PreviewOut, Stage0PreviewRequest, Stage0RunOut
...
from app.services.ppr_stage0_cohort_service import Stage0ConflictError, Stage0Error, Stage0NotFoundError, Stage0ValidationError, freeze_stage0_cohort, get_stage0_blockers, get_stage0_run, preview_stage0_cohort
...
def preview(payload: Stage0PreviewRequest, user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
    ...
    try:
        with engine.connect() as conn:
            ...
            return preview_stage0_cohort(conn, source_batch_id=payload.source_batch_id, supplemental_of_run_id=payload.supplemental_of_run_id, scope=scope)
...
def get_run(run_id: int = Path(ge=1), user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
    actor = require_ppr_stage0_cohort_manage(user); scope = _scope(user, actor)
    try:
        with engine.connect() as conn: return get_stage0_run(conn, run_id=run_id, scope=scope)
    except Stage0Error as exc: raise _error(exc)
