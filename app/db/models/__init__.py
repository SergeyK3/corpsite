"""Corpsite SQLAlchemy ORM models."""
from app.db.base import Base
from app.db.models.org_unit_allowed_positions import OrgUnitAllowedPosition
from app.db.models.aliases import OrgUnitAlias, PositionAlias
from app.db.models.employee_identity import IDENTITY_TYPE_IIN, EmployeeIdentity
from app.db.models.personnel_orders import (
    ATTACHMENT_KIND_BASIS_DOCUMENT,
    ATTACHMENT_KIND_SIGNED_SCAN,
    ATTACHMENT_KIND_UNSIGNED_DRAFT,
    BASIS_TYPE_COMMISSION_PROTOCOL,
    BASIS_TYPE_COURT_ACT,
    BASIS_TYPE_MANAGEMENT_SUBMISSION,
    BASIS_TYPE_MEDICAL_CONCLUSION,
    BASIS_TYPE_MEMO,
    BASIS_TYPE_OTHER,
    BASIS_TYPE_PERSONAL_APPLICATION,
    BASIS_TYPES,
    ITEM_BLOCK_TYPE_BASIS,
    ITEM_BLOCK_TYPE_BODY,
    ITEM_BLOCK_TYPES,
    ITEM_STATUS_ACTIVE,
    ITEM_STATUS_VOIDED,
    LOCALE_KK,
    LOCALE_RU,
    MVP_ITEM_TYPE_CODES,
    MVP_ORDER_TYPE_CODES,
    ORDER_BLOCK_TYPE_CLOSING,
    ORDER_BLOCK_TYPE_PREAMBLE,
    ORDER_BLOCK_TYPE_TITLE,
    ORDER_BLOCK_TYPES,
    ORDER_CLASS_PERSONNEL,
    ORDER_STATUS_DRAFT,
    ORDER_STATUS_READY_FOR_SIGNATURE,
    ORDER_STATUS_REGISTERED,
    ORDER_STATUS_SIGNED,
    ORDER_STATUS_VOIDED,
    ORDER_TYPE_COMPOSITE,
    ORDER_TYPE_CONCURRENT_DUTY_END,
    ORDER_TYPE_CONCURRENT_DUTY_START,
    ORDER_TYPE_HIRE,
    ORDER_TYPE_TERMINATION,
    ORDER_TYPE_TRANSFER,
    PRINT_FORMAT_DOCX,
    PRINT_FORMAT_PDF,
    REVIEW_STATUS_CURRENT,
    REVIEW_STATUS_GENERATION_FAILED,
    REVIEW_STATUS_REVIEW_REQUIRED,
    REVIEW_STATUS_STALE,
    REVIEW_STATUSES,
    SOURCE_MODE_DIGITAL,
    SOURCE_MODE_PAPER,
    STORAGE_TYPE_LOCAL_SHARE,
    STORAGE_TYPE_URL,
    PersonnelOrder,
    PersonnelOrderAttachment,
    PersonnelOrderEditorialBlock,
    PersonnelOrderItem,
    PersonnelOrderItemBasis,
    PersonnelOrderItemEditorialBlock,
    PersonnelOrderLocalizedText,
    PersonnelOrderPrint,
)
from app.db.models.operational_orders import (
    OperationalOrderBilingualReconciliation,
    OperationalOrderClarification,
    OperationalOrderContentConfirmation,
    OperationalOrderDocument,
    OperationalOrderDocumentLocalization,
    OperationalOrderDocumentVersion,
    OperationalOrderDraftAudit,
    OperationalOrderDraftBlock,
    OperationalOrderDraftWorkspace,
    OperationalOrderLifecycleAudit,
    OperationalOrderPromotion,
    OperationalOrderPromotionAudit,
    OperationalOrderSigningAuthority,
    OperationalOrderTextProvenance,
    OperationalOrderTranslationAssignment,
)
from app.db.models.operational_order_archive_import import (
    IMPORT_BATCH_STATUSES,
    INITIAL_REVIEW_STATES,
    OperationalOrderImportBatch,
    OperationalOrderImportRow,
)
from app.db.models.personnel_record_metadata import (
    HR_RELATIONSHIP_CONTEXTS,
    HR_RELATIONSHIP_UNKNOWN,
    PPR_ENVELOPE_INITIAL_HR_RELATIONSHIP_CONTEXT,
    PPR_ENVELOPE_INITIAL_LIFECYCLE_STATE,
    PPR_ENVELOPE_INITIAL_VERSION,
    PPR_LIFECYCLE_CREATED,
    PPR_LIFECYCLE_STATES,
    PersonnelRecordMetadata,
)
from app.db.models.ppr_stage0_cohort import (
    PprStage0CohortBlocker,
    PprStage0CohortParticipant,
    PprStage0CohortRun,
)
from app.db.models.ppr_migration_status_projection import (
    PPR_MIGRATION_SECTIONS,
    PPR_MIGRATION_STATUS_CODES,
    PprMigrationSectionStatusProjection,
    PprMigrationStatusUniverse,
)
from app.db.models.ppr_manual_correction_events import (
    PPR_CORRECTION_EVENT_TYPE,
    PPR_CORRECTION_ORIGIN_REASON_CODES,
    PPR_CORRECTION_ORIGIN_STATUSES,
    PPR_CORRECTION_SECTIONS,
    PPR_PROJECTION_OUTBOX_ERROR_CODES,
    PPR_PROJECTION_OUTBOX_JOB_KINDS,
    PPR_PROJECTION_OUTBOX_STATES,
    PprMigrationProjectionOutbox,
    PprSectionManualCorrectionEvent,
)
from app.db.models.personnel_migration import (
    DOMAIN_CODE_EDUCATION,
    EDUCATION_KINDS,
    EVENT_TYPE_EDUCATION_MIGRATED,
    EVENT_TYPE_EDUCATION_SUPERSEDED,
    EVENT_TYPE_EDUCATION_VERIFIED,
    EVENT_TYPE_EDUCATION_VOIDED,
    ITEM_STATUS_COMMITTED,
    ITEM_STATUS_DRAFT,
    ITEM_STATUS_FAILED,
    ITEM_STATUS_SUPERSEDED,
    ITEM_STATUS_VOIDED,
    ITEM_STATUSES,
    LIFECYCLE_STATUS_ACTIVE,
    LIFECYCLE_STATUS_DRAFT,
    LIFECYCLE_STATUS_SUPERSEDED,
    LIFECYCLE_STATUS_VOIDED,
    LIFECYCLE_STATUSES,
    PERSON_STATUS_FACT_KIND_DISABILITY,
    PERSON_STATUS_FACT_KIND_PENSION,
    PERSON_STATUS_FACT_KINDS,
    PERSON_STATUS_FACT_REVIEW_READY,
    PERSON_STATUS_FACT_REVIEW_REQUIRED,
    PERSON_STATUS_FACT_REVIEW_STATUSES,
    PersonEducation,
    PersonStatusFact,
    PersonnelMigrationDomain,
    PersonnelMigrationItem,
    PersonnelMigrationRun,
    PersonnelRecordEvent,
    PersonTraining,
    RUN_STATUS_COMMITTED,
    RUN_STATUS_DRAFT,
    RUN_STATUS_FAILED,
    RUN_STATUS_VOIDED,
    RUN_STATUSES,
    TRAINING_KINDS,
    VERIFICATION_STATUS_PENDING,
    VERIFICATION_STATUS_VERIFIED,
    VERIFICATION_STATUSES,
)
from app.db.models.control_list_mapping import (
    ControlListMappingProfile,
    ControlListMappingProfileColumn,
    ControlListMappingProfileSheet,
    PROFILE_STATUS_ACTIVE,
    PROFILE_STATUS_ARCHIVED,
    PROFILE_STATUS_DRAFT,
)
from app.db.models.control_list_import import (
    ControlListImportCell,
    ControlListImportRow,
    ControlListImportRun,
    ControlListImportSheet,
    EMPLOYMENT_MODE_CONCURRENT,
    EMPLOYMENT_MODE_PRIMARY,
    EMPLOYMENT_MODE_UNKNOWN,
    INFERRED_TYPE_COMPOSITE_TEXT,
    INFERRED_TYPE_EMPTY,
    INFERRED_TYPE_ERROR,
    INFERRED_TYPE_EXCEL_DATE,
    INFERRED_TYPE_FORMULA,
    INFERRED_TYPE_IIN_CANDIDATE,
    INFERRED_TYPE_INTEGER,
    INFERRED_TYPE_PHONE_CANDIDATE,
    INFERRED_TYPE_TEXT,
    PERSONNEL_CATEGORY_DOCTOR,
    PERSONNEL_CATEGORY_JUNIOR_MEDICAL_STAFF,
    PERSONNEL_CATEGORY_NURSING_STAFF,
    PERSONNEL_CATEGORY_OTHER_STAFF,
    PERSONNEL_CATEGORY_UNKNOWN,
    ROW_KIND_DATA,
    ROW_KIND_EMPTY,
    ROW_KIND_FOOTER,
    ROW_KIND_HEADER,
    ROW_KIND_SECTION_HEADER,
    ROW_KIND_TITLE,
    ROW_KIND_UNKNOWN,
    RUN_STATUS_CANCELLED,
    RUN_STATUS_FAILED,
    RUN_STATUS_STAGED,
    SHEET_PURPOSE_DECLARATION,
    SHEET_PURPOSE_PERSONNEL_CONTROL_LIST,
    SHEET_PURPOSE_UNKNOWN,
    SHEET_STATUS_ANALYZED,
    SHEET_STATUS_EXCLUDED,
)
from app.db.models.control_list_apply_execution import (
    APPLY_ACTION_STATUS_DEFERRED,
    APPLY_ACTION_STATUS_FAILED,
    APPLY_ACTION_STATUS_PENDING,
    APPLY_ACTION_STATUS_RUNNING,
    APPLY_ACTION_STATUS_SKIPPED,
    APPLY_ACTION_STATUS_SUCCEEDED,
    APPLY_RUN_STATUS_FAILED,
    APPLY_RUN_STATUS_PARTIALLY_SUCCEEDED,
    APPLY_RUN_STATUS_PENDING,
    APPLY_RUN_STATUS_RUNNING,
    APPLY_RUN_STATUS_SUCCEEDED,
    ControlListApplyAction,
    ControlListApplyRun,
)
from app.db.models.hr_monthly_reference import (
    COMPARISON_RUN_CANCELLED,
    COMPARISON_RUN_COMPLETED,
    COMPARISON_RUN_FAILED,
    COMPARISON_RUN_RUNNING,
    DIFFERENCE_BUSINESS_NEVER_CONFIRMED,
    DIFFERENCE_BUSINESS_PERIOD_CHANGED,
    DIFFERENCE_LIFECYCLE_CONFIRMED,
    DIFFERENCE_LIFECYCLE_DETECTED,
    DIFFERENCE_LIFECYCLE_REJECTED,
    DIFFERENCE_LIFECYCLE_SUPERSEDED,
    HrComparisonRun,
    HrConfirmedChange,
    HrDetectedDifference,
    HrDifferenceOriginType,
    HrMonthlyReference,
    HrMonthlyReferenceEntry,
    HrReferenceVersionEvent,
    MRD_STATUS_ACTIVE,
    MRD_STATUS_CLOSED,
    ORIGIN_DATA_REPAIR,
    ORIGIN_IMPORT_COMPARE,
    ORIGIN_MANUAL_EDIT,
    ORIGIN_MRD_FORK,
    ORIGIN_SYSTEM_RECALC,
    REFERENCE_EVENT_ACTIVATE,
    REFERENCE_EVENT_CLOSE,
    REFERENCE_EVENT_FORK_PERIOD,
    REFERENCE_EVENT_FORK_VERSION,
)
from app.db.models.hr_import import (
    BATCH_STATUS_APPLIED,
    BATCH_STATUS_APPLY_PENDING,
    BATCH_STATUS_CANCELLED,
    BATCH_STATUS_FAILED,
    BATCH_STATUS_IN_REVIEW,
    BATCH_STATUS_PARSED,
    BATCH_STATUS_PARTIALLY_APPLIED,
    BATCH_STATUS_UPLOADED,
    HrImportBatch,
    HrImportDocumentCandidate,
    HrImportIdentityQualityState,
    HrImportIdentityReviewEvent,
    HrImportRow,
    IDENTITY_QUALITY_REASON_CODES,
    IDENTITY_QUALITY_POLICY_CODES,
    IDENTITY_QUALITY_STATES,
    IDENTITY_REVIEW_ACTOR_HR,
    IDENTITY_REVIEW_ACTOR_SYSTEM,
    IDENTITY_REVIEW_ACTOR_TYPES,
    IDENTITY_REVIEW_EVENT_TYPES,
    CLASSIFICATION_DECLARATION,
    CLASSIFICATION_DUPLICATE_IIN,
    CLASSIFICATION_INVALID_IIN,
    CLASSIFICATION_NORMAL,
    CLASSIFICATION_PART_TIME,
    CLASSIFICATION_SUMMARY_ROW,
    MATCH_STATUS_AUTO,
    MATCH_STATUS_INVALID,
    MATCH_STATUS_NO_MATCH,
    MATCH_STATUS_NOT_PROCESSED,
    MATCH_STATUS_REVIEW,
    MATCH_STATUS_SKIPPED,
    REVIEW_STATUS_APPROVED,
    REVIEW_STATUS_MERGED,
    REVIEW_STATUS_PENDING,
    REVIEW_STATUS_REJECTED,
    SOURCE_TYPE_HR_CONTROL_LIST,
)
from app.db.models.personnel_verification import (
    ALLOWED_CONTROL_POINTS,
    ATTESTATION_DECISION_REJECTED,
    ATTESTATION_DECISION_VERIFIED,
    ATTESTATION_DECISIONS,
    CONTROL_POINT_EMPLOYMENT_EPISODE,
    CONTROL_POINT_MEDICAL_CATEGORY,
    OBJECT_TYPE_PERSON_EXTERNAL_EMPLOYMENT,
    POLICY_STATUS_ACTIVE,
    POLICY_STATUS_DRAFT,
    POLICY_STATUS_INACTIVE,
    POLICY_STATUSES,
    TASK_STATUS_CANCELLED,
    TASK_STATUS_COMPLETED,
    TASK_STATUS_PENDING,
    TASK_STATUS_REJECTED,
    TASK_STATUSES,
    VerificationAttestation,
    VerificationPolicy,
    VerificationTask,
)
from app.db.models.personnel_intake import (
    PersonnelIntakeDraft,
    PersonnelIntakeLink,
    PersonnelIntakeReconciliationDecision,
    PersonnelIntakeSectionReview,
    PersonnelIntakeTransfer,
)
from app.db.models.person_photos import (
    BLOCKER_CODE_INTAKE_PHOTO_UNAVAILABLE,
    BLOCKER_CODE_PHOTO_CANONICALIZATION_FAILED,
    BLOCKER_CODES,
    CANONICALIZATION_MODE_BACKFILL,
    CANONICALIZATION_MODE_HIRE_APPLY,
    CANONICALIZATION_MODE_TRANSFER,
    CANONICALIZATION_MODES,
    MAX_PHOTO_BYTE_SIZE,
    MIME_TYPE_JPEG,
    PersonPhoto,
    PersonPhotoSource,
    PersonnelApplicationBlocker,
    SOURCE_KIND_INTAKE,
    SOURCE_KIND_MANUAL_UPLOAD,
    SOURCE_KINDS,
)

__all__ = [
    "Base",
    "ControlListImportCell",
    "ControlListImportRow",
    "ControlListImportRun",
    "ControlListImportSheet",
    "ControlListMappingProfile",
    "ControlListMappingProfileColumn",
    "ControlListMappingProfileSheet",
    "PROFILE_STATUS_ACTIVE",
    "PROFILE_STATUS_ARCHIVED",
    "PROFILE_STATUS_DRAFT",
    "EMPLOYMENT_MODE_CONCURRENT",
    "EMPLOYMENT_MODE_PRIMARY",
    "EMPLOYMENT_MODE_UNKNOWN",
    "INFERRED_TYPE_COMPOSITE_TEXT",
    "INFERRED_TYPE_EMPTY",
    "INFERRED_TYPE_ERROR",
    "INFERRED_TYPE_EXCEL_DATE",
    "INFERRED_TYPE_FORMULA",
    "INFERRED_TYPE_IIN_CANDIDATE",
    "INFERRED_TYPE_INTEGER",
    "INFERRED_TYPE_PHONE_CANDIDATE",
    "INFERRED_TYPE_TEXT",
    "PERSONNEL_CATEGORY_DOCTOR",
    "PERSONNEL_CATEGORY_JUNIOR_MEDICAL_STAFF",
    "PERSONNEL_CATEGORY_NURSING_STAFF",
    "PERSONNEL_CATEGORY_OTHER_STAFF",
    "PERSONNEL_CATEGORY_UNKNOWN",
    "ROW_KIND_DATA",
    "ROW_KIND_EMPTY",
    "ROW_KIND_FOOTER",
    "ROW_KIND_HEADER",
    "ROW_KIND_SECTION_HEADER",
    "ROW_KIND_TITLE",
    "ROW_KIND_UNKNOWN",
    "RUN_STATUS_CANCELLED",
    "RUN_STATUS_FAILED",
    "RUN_STATUS_STAGED",
    "SHEET_PURPOSE_DECLARATION",
    "SHEET_PURPOSE_PERSONNEL_CONTROL_LIST",
    "SHEET_PURPOSE_UNKNOWN",
    "SHEET_STATUS_ANALYZED",
    "SHEET_STATUS_EXCLUDED",
    "ATTACHMENT_KIND_BASIS_DOCUMENT",
    "ATTACHMENT_KIND_SIGNED_SCAN",
    "ATTACHMENT_KIND_UNSIGNED_DRAFT",
    "BASIS_TYPE_COMMISSION_PROTOCOL",
    "BASIS_TYPE_COURT_ACT",
    "BASIS_TYPE_MANAGEMENT_SUBMISSION",
    "BASIS_TYPE_MEDICAL_CONCLUSION",
    "BASIS_TYPE_MEMO",
    "BASIS_TYPE_OTHER",
    "BASIS_TYPE_PERSONAL_APPLICATION",
    "BASIS_TYPES",
    "EmployeeIdentity",
    "ITEM_BLOCK_TYPE_BASIS",
    "ITEM_BLOCK_TYPE_BODY",
    "ITEM_BLOCK_TYPES",
    "ITEM_STATUS_ACTIVE",
    "ITEM_STATUS_VOIDED",
    "LOCALE_KK",
    "LOCALE_RU",
    "MVP_ITEM_TYPE_CODES",
    "MVP_ORDER_TYPE_CODES",
    "ORDER_BLOCK_TYPE_CLOSING",
    "ORDER_BLOCK_TYPE_PREAMBLE",
    "ORDER_BLOCK_TYPE_TITLE",
    "ORDER_BLOCK_TYPES",
    "ORDER_CLASS_PERSONNEL",
    "ORDER_STATUS_DRAFT",
    "ORDER_STATUS_READY_FOR_SIGNATURE",
    "ORDER_STATUS_REGISTERED",
    "ORDER_STATUS_SIGNED",
    "ORDER_STATUS_VOIDED",
    "ORDER_TYPE_COMPOSITE",
    "ORDER_TYPE_CONCURRENT_DUTY_END",
    "ORDER_TYPE_CONCURRENT_DUTY_START",
    "ORDER_TYPE_HIRE",
    "ORDER_TYPE_TERMINATION",
    "ORDER_TYPE_TRANSFER",
    "PersonnelOrder",
    "PersonnelOrderAttachment",
    "PersonnelOrderEditorialBlock",
    "PersonnelOrderItem",
    "PersonnelOrderItemBasis",
    "PersonnelOrderItemEditorialBlock",
    "PersonnelOrderLocalizedText",
    "PersonnelOrderPrint",
    "PRINT_FORMAT_DOCX",
    "PRINT_FORMAT_PDF",
    "REVIEW_STATUS_CURRENT",
    "REVIEW_STATUS_GENERATION_FAILED",
    "REVIEW_STATUS_REVIEW_REQUIRED",
    "REVIEW_STATUS_STALE",
    "REVIEW_STATUSES",
    "SOURCE_MODE_DIGITAL",
    "SOURCE_MODE_PAPER",
    "STORAGE_TYPE_LOCAL_SHARE",
    "STORAGE_TYPE_URL",
    "HrImportBatch",
    "HrImportDocumentCandidate",
    "HrImportIdentityQualityState",
    "HrImportIdentityReviewEvent",
    "HrImportRow",
    "IDENTITY_QUALITY_STATES",
    "IDENTITY_QUALITY_REASON_CODES",
    "IDENTITY_QUALITY_POLICY_CODES",
    "IDENTITY_REVIEW_ACTOR_HR",
    "IDENTITY_REVIEW_ACTOR_SYSTEM",
    "IDENTITY_REVIEW_ACTOR_TYPES",
    "IDENTITY_REVIEW_EVENT_TYPES",
    "IDENTITY_TYPE_IIN",
    "OrgUnitAlias",
    "OperationalOrderBilingualReconciliation",
    "OperationalOrderClarification",
    "OperationalOrderContentConfirmation",
    "OperationalOrderDocument",
    "OperationalOrderDocumentLocalization",
    "OperationalOrderDocumentVersion",
    "OperationalOrderDraftAudit",
    "OperationalOrderDraftBlock",
    "OperationalOrderDraftWorkspace",
    "OperationalOrderPromotion",
    "OperationalOrderPromotionAudit",
    "OperationalOrderTextProvenance",
    "OperationalOrderTranslationAssignment",
    "OperationalOrderImportBatch",
    "OperationalOrderImportRow",
    "IMPORT_BATCH_STATUSES",
    "INITIAL_REVIEW_STATES",
    "OrgUnitAllowedPosition",
    "PositionAlias",
    "SOURCE_TYPE_HR_CONTROL_LIST",
    "BATCH_STATUS_UPLOADED",
    "BATCH_STATUS_PARSED",
    "BATCH_STATUS_IN_REVIEW",
    "BATCH_STATUS_APPLY_PENDING",
    "BATCH_STATUS_APPLIED",
    "BATCH_STATUS_PARTIALLY_APPLIED",
    "BATCH_STATUS_FAILED",
    "BATCH_STATUS_CANCELLED",
    "CLASSIFICATION_NORMAL",
    "CLASSIFICATION_INVALID_IIN",
    "CLASSIFICATION_DUPLICATE_IIN",
    "CLASSIFICATION_DECLARATION",
    "CLASSIFICATION_SUMMARY_ROW",
    "CLASSIFICATION_PART_TIME",
    "MATCH_STATUS_NOT_PROCESSED",
    "MATCH_STATUS_AUTO",
    "MATCH_STATUS_REVIEW",
    "MATCH_STATUS_NO_MATCH",
    "MATCH_STATUS_INVALID",
    "MATCH_STATUS_SKIPPED",
    "REVIEW_STATUS_PENDING",
    "REVIEW_STATUS_APPROVED",
    "REVIEW_STATUS_REJECTED",
    "REVIEW_STATUS_MERGED",
    "HR_RELATIONSHIP_CONTEXTS",
    "HR_RELATIONSHIP_UNKNOWN",
    "PPR_ENVELOPE_INITIAL_HR_RELATIONSHIP_CONTEXT",
    "PPR_ENVELOPE_INITIAL_LIFECYCLE_STATE",
    "PPR_ENVELOPE_INITIAL_VERSION",
    "PPR_LIFECYCLE_CREATED",
    "PPR_LIFECYCLE_STATES",
    "PersonnelRecordMetadata",
    "DOMAIN_CODE_EDUCATION",
    "EDUCATION_KINDS",
    "EVENT_TYPE_EDUCATION_MIGRATED",
    "EVENT_TYPE_EDUCATION_SUPERSEDED",
    "EVENT_TYPE_EDUCATION_VERIFIED",
    "EVENT_TYPE_EDUCATION_VOIDED",
    "ITEM_STATUS_COMMITTED",
    "ITEM_STATUS_DRAFT",
    "ITEM_STATUS_FAILED",
    "ITEM_STATUS_SUPERSEDED",
    "ITEM_STATUS_VOIDED",
    "ITEM_STATUSES",
    "LIFECYCLE_STATUS_ACTIVE",
    "LIFECYCLE_STATUS_DRAFT",
    "LIFECYCLE_STATUS_SUPERSEDED",
    "LIFECYCLE_STATUS_VOIDED",
    "LIFECYCLE_STATUSES",
    "PERSON_STATUS_FACT_KIND_DISABILITY",
    "PERSON_STATUS_FACT_KIND_PENSION",
    "PERSON_STATUS_FACT_KINDS",
    "PERSON_STATUS_FACT_REVIEW_READY",
    "PERSON_STATUS_FACT_REVIEW_REQUIRED",
    "PERSON_STATUS_FACT_REVIEW_STATUSES",
    "PersonEducation",
    "PersonStatusFact",
    "PersonnelMigrationDomain",
    "PersonnelMigrationItem",
    "PersonnelMigrationRun",
    "PersonnelRecordEvent",
    "PersonTraining",
    "RUN_STATUS_COMMITTED",
    "RUN_STATUS_DRAFT",
    "RUN_STATUS_FAILED",
    "RUN_STATUS_VOIDED",
    "RUN_STATUSES",
    "TRAINING_KINDS",
    "VERIFICATION_STATUS_PENDING",
    "VERIFICATION_STATUS_VERIFIED",
    "VERIFICATION_STATUSES",
    "HrComparisonRun",
    "HrConfirmedChange",
    "HrDetectedDifference",
    "HrDifferenceOriginType",
    "HrMonthlyReference",
    "HrMonthlyReferenceEntry",
    "HrReferenceVersionEvent",
    "MRD_STATUS_ACTIVE",
    "MRD_STATUS_CLOSED",
    "DIFFERENCE_LIFECYCLE_DETECTED",
    "DIFFERENCE_LIFECYCLE_CONFIRMED",
    "DIFFERENCE_LIFECYCLE_REJECTED",
    "DIFFERENCE_LIFECYCLE_SUPERSEDED",
    "DIFFERENCE_BUSINESS_NEVER_CONFIRMED",
    "DIFFERENCE_BUSINESS_PERIOD_CHANGED",
    "ORIGIN_IMPORT_COMPARE",
    "ORIGIN_MANUAL_EDIT",
    "ORIGIN_SYSTEM_RECALC",
    "ORIGIN_MRD_FORK",
    "ORIGIN_DATA_REPAIR",
    "COMPARISON_RUN_RUNNING",
    "COMPARISON_RUN_COMPLETED",
    "COMPARISON_RUN_FAILED",
    "COMPARISON_RUN_CANCELLED",
    "REFERENCE_EVENT_FORK_VERSION",
    "REFERENCE_EVENT_FORK_PERIOD",
    "REFERENCE_EVENT_CLOSE",
    "REFERENCE_EVENT_ACTIVATE",
    "VerificationPolicy",
    "VerificationTask",
    "VerificationAttestation",
    "ALLOWED_CONTROL_POINTS",
    "CONTROL_POINT_EMPLOYMENT_EPISODE",
    "CONTROL_POINT_MEDICAL_CATEGORY",
    "OBJECT_TYPE_PERSON_EXTERNAL_EMPLOYMENT",
    "POLICY_STATUS_DRAFT",
    "POLICY_STATUS_ACTIVE",
    "POLICY_STATUS_INACTIVE",
    "POLICY_STATUSES",
    "TASK_STATUS_PENDING",
    "TASK_STATUS_COMPLETED",
    "TASK_STATUS_REJECTED",
    "TASK_STATUS_CANCELLED",
    "TASK_STATUSES",
    "ATTESTATION_DECISION_VERIFIED",
    "ATTESTATION_DECISION_REJECTED",
    "ATTESTATION_DECISIONS",
    "PersonnelIntakeDraft",
    "PersonnelIntakeLink",
    "PersonnelIntakeReconciliationDecision",
    "PersonnelIntakeSectionReview",
    "PersonnelIntakeTransfer",
    "PersonPhoto",
    "PersonPhotoSource",
    "PersonnelApplicationBlocker",
    "SOURCE_KIND_INTAKE",
    "SOURCE_KIND_MANUAL_UPLOAD",
    "SOURCE_KINDS",
    "CANONICALIZATION_MODE_TRANSFER",
    "CANONICALIZATION_MODE_HIRE_APPLY",
    "CANONICALIZATION_MODE_BACKFILL",
    "CANONICALIZATION_MODES",
    "BLOCKER_CODE_INTAKE_PHOTO_UNAVAILABLE",
    "BLOCKER_CODE_PHOTO_CANONICALIZATION_FAILED",
    "BLOCKER_CODES",
    "MIME_TYPE_JPEG",
    "MAX_PHOTO_BYTE_SIZE",
]
