...
import { runTitleLabel, scheduleTypeLabel, uiFieldLabel } from "@/lib/i18n";
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {showJournalLink && result.run_id ? (
          ...
          >
            Открыть журнал запуска — {runTitleLabel(result.run_id)}
          </Link>
        ) : null}
      </div>
      ...
      <div className="mt-4 rounded-xl border border-amber-200 dark:border-amber-900/40 bg-amber-50/80 dark:bg-amber-950/20 px-4 py-3">
        ...
        <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-amber-900/90 dark:text-amber-200/90">
          <span>
            <span className="text-amber-700/80 dark:text-amber-400/80">run_for_date:</span>{" "}
            {resolved.run_for_date ? fmtDate(resolved.run_for_date) : "—"}
          </span>
          <span>
            <span className="text-amber-700/80 dark:text-amber-400/80">occurrence_date:</span>{" "}
            {occurrenceDate ? fmtDate(occurrenceDate) : "—"}
          </span>
          {firstItemDates ? (
            <>
              <span>
                <span className="text-amber-700/80 dark:text-amber-400/80">due_date:</span>{" "}
                {firstItemDates.due_date_label}
              </span>
            </>
          ) : null}
          <span>
            <span className="text-amber-700/80 dark:text-amber-400/80">тип:</span>{" "}
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        <StatBlock label="templates_total" value={stats.templates_total ?? 0} />
        <StatBlock label="templates_due" value={stats.templates_due ?? 0} />
        <StatBlock label="created" value={stats.created ?? 0} />
        <StatBlock label="deduped" value={stats.deduped ?? 0} />
        <StatBlock label="errors" value={stats.errors ?? 0} />
      </div>
      ...
      {reviewRows.length > 0 ? (
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="bg-zinc-100/80 dark:bg-zinc-950/80 text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
              <tr>
                ...
                <th className="px-3 py-2 font-medium">title_final</th>
              </tr>
            </thead>
        ...
        <p className="mt-4 text-sm text-zinc-600 dark:text-zinc-400">
          Нет элементов в scope (templates_due = 0) или items ещё не загружены.
        </p>
      )}
    </section>
  );
