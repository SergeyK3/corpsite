...
type Run = { run: { stage_run_id: number; status: string; current_position: number; paused_operation?: string | null; acceptance_outcome?: Values }; participants: Participant[]; counts: Record<string, number>; organization_timezone?: string };
...
function ParticipantCard({ participant, accepted, onSkip, busy }: { participant: Participant; accepted: boolean; onSkip: (participant: Participant, reason: string) => void; busy: boolean }) {
  ...
  return <article className="space-y-3 rounded-lg border bg-white p-4 dark:bg-zinc-950" aria-label={`Участник ${participant.position}`}>
    ...
    {participant.error_code ? <p role="status" className="rounded border border-amber-300 bg-amber-50 p-2 text-sm">Обработка остановлена: проверьте исходные данные. Код причины доступен в технических сведениях.</p> : null}
...
export default function Stage2EducationPanel({ cohortRunId }: { cohortRunId?: number }) {
  ...
  return <section className="space-y-4 rounded-xl border border-sky-300 bg-sky-50/40 p-4 dark:border-sky-900 dark:bg-sky-950/20" data-testid="stage2-education-panel">
    ...
    {run ? <><div className="rounded border bg-white p-3 text-sm dark:bg-zinc-950"><p><b>Статус: </b>{stateLabel[run.run.status] ?? run.run.status}</p><p>Обработано: {run.counts.COMPLETED ?? 0} из {run.participants.length}; исключено: {run.counts.SKIPPED_BY_DECISION ?? 0}.</p>{accepted ? <p className="mt-2 font-medium">Принятие завершено. Повторное открытие показывает сохранённый результат без повторной записи.</p> : null}<details className="mt-1 text-xs"><summary>Технические сведения</summary>Код состояния: {run.run.status}; ID прогона: {id}</details></div>
      <div className="flex flex-wrap gap-2">{run.run.status === "DRY_RUN_COMPLETED" ? <button type="button" disabled={busy} onClick={() => void action("/approve")} className="rounded bg-blue-700 px-3 py-2 text-sm text-white">Утвердить запуск</button> : null}{["APPROVED", "RUNNING"].includes(run.run.status) ? <button type="button" disabled={busy} onClick={() => void action("/execute-next")} className="rounded bg-blue-700 px-3 py-2 text-sm text-white">Обработать следующего</button> : null}{run.run.status === "PAUSED_ON_ERROR" && run.run.paused_operation === "PARTICIPANT_EXECUTION" ? <button type="button" disabled={busy} onClick={() => void action("/resume")} className="rounded bg-amber-700 px-3 py-2 text-sm text-white">Продолжить обработку</button> : null}{run.run.status === "PAUSED_ON_ERROR" && run.run.paused_operation === "ACCEPTANCE" ? <button type="button" disabled={busy} onClick={() => void showAcceptance()} className="rounded bg-amber-700 px-3 py-2 text-sm text-white">Повторить принятие</button> : null}{run.run.status === "COMPLETED_PENDING_REVIEW" ? <button type="button" disabled={busy} onClick={() => void showAcceptance()} className="rounded bg-emerald-700 px-3 py-2 text-sm text-white">Принять этап</button> : null}</div>
