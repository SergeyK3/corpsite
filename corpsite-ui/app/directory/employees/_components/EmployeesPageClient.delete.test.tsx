...
describe("EmployeesPageClient staff hard-delete", () => {
  it("does not show delete button even for a stale enabled ADMIN capability", async () => {
  ...
  it("never calls legacy delete from the staff UI", async () => {
    renderWithMe({ user_id: 1, role_id: 2, is_system_admin: true, can_hard_delete_employee: true });
  ...
  it("does not expose the action when the stale API mock would fail", async () => {
    renderWithMe({ user_id: 1, role_id: 2, is_system_admin: true, can_hard_delete_employee: true });
